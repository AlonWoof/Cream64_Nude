    ...
    if (m->health >= 0x100) {
        ...
        if (!noExit) {
            ...
            fadeout_level_music(126);
                    ...
                    if(gMarioState->currentCostume == 77) {
    ...
    if (m->action != ACT_GETTING_BLOWN && capFlag != 0) {
        ...
        switch (capFlag) {
            case MARIO_VANISH_CAP: if(gCurrLevelNum == LEVEL_VCUTM)  {
                    capTime = VC_TIME; break;
                }
                else {
                    capTime = VC_TIME; capMusic = SEQUENCE_ARGS(4, SEQ_EVENT_VANISH  ); break;
                }
            case MARIO_METAL_CAP: if(gCurrLevelNum == LEVEL_BITDW || gCurrLevelNum == LEVEL_COTMC)  {
                    capTime = MC_TIME; break;
                }
                else {
                    capTime = MC_TIME; capMusic = SEQUENCE_ARGS(4, SEQ_EVENT_METAL_CAP  ); break;
                }
            case MARIO_WING_CAP: if(gCurrLevelNum == LEVEL_WMOTR || gCurrLevelNum == LEVEL_TOTWC)  {
                    capTime = WC_TIME; break;
                }
                else {
                    capTime = WC_TIME; capMusic = SEQUENCE_ARGS(4, SEQ_EVENT_POWERUP  ); break;
                }
        }
